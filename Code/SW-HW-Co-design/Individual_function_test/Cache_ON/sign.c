...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
  ...
  /* Expand matrix */
  polyvec_matrix_expand_Hardware(mat, rho);//polyvec_matrix_expand(mat, rho);
  ...
  polyvec_matrix_PWM_Hardware(mat_temp, mat, &s1hat);
...
rej:
  ...
  /* Matrix-vector multiplication */
  polyveck mat_temp[L];
  polyvec_matrix_PWM_Hardware(mat_temp, mat, &z);
  polyvec_matrix_add_Hardware(&w1, mat_temp);
                       ...
                       const uint8_t *pk)
{
  ...
  polyvecl_ntt_Hardware(&z);//polyvecl_ntt(&z);
  polyvec_matrix_PWM_Hardware(mat_temp, mat, &z);
  polyvec_matrix_add_Hardware(&w1, mat_temp);//polyvec_matrix_pointwise_montgomery(&w1, mat, &z);
